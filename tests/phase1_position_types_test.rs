...
fn test_relative_road_position_xml_roundtrip() {
    ...
    // Deserialize back
    let deserialized: RelativeRoadPosition = quick_xml::de::from_str(&xml)
...
fn test_relative_lane_position_xml_roundtrip() {
    ...
    // Deserialize back
    let deserialized: RelativeLanePosition = quick_xml::de::from_str(&xml)
        .expect("Failed to deserialize");
...
fn test_relative_road_position_with_orientation() {
    ...
    let pos = RelativeRoadPosition::with_orientation(
    ...
    assert_eq!(
        deserialized.orientation.unwrap().h.unwrap().as_literal().unwrap(),
        &1.57
    );
}
...
fn test_parameter_support() {
    ...
    assert_eq!(deserialized.entity_ref.as_parameter().unwrap(), "TargetEntity");
...
fn test_default_implementations() {
    ...
    assert_eq!(rel_road_default.entity_ref.as_literal().unwrap(), "DefaultEntity");
...
fn test_builder_methods() {
    ...
    let pos2 = RelativeRoadPosition::with_orientation("Vehicle2".to_string(), 10.0, -1.0, orientation);

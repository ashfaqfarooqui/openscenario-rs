...
use openscenario_rs::types::{
    ...
    entities::{
        ByName, ByObjectType, ByType, EntityDistribution, EntityDistributionEntry,
...
fn test_entity_selection_by_object_type() {
    ...
    assert_eq!(selection.by_type.as_ref().unwrap().object_type, ObjectType::Vehicle);
...
fn test_entity_selection_by_name() {
    ...
    assert_eq!(
        selection.by_name.as_ref().unwrap().name.as_literal().unwrap(),
        "Ego*"
    );
...
fn test_entity_selection_by_type_and_name() {
    ...
    assert!(selection.by_type.is_some());
    assert!(selection.by_name.is_some());
    assert_eq!(selection.by_type.as_ref().unwrap().object_type, ObjectType::Pedestrian);
    assert_eq!(
        selection.by_name.as_ref().unwrap().name.as_literal().unwrap(),
        "Walker*"
    );
...
fn test_entity_distribution_entry() {
    ...
    assert_eq!(default_entry.entity_ref.as_literal().unwrap(), "DefaultEntity");
...
fn test_complex_entity_selection_scenario() {
    ...
    assert_eq!(
        vehicle_template.properties.unwrap().properties.len(),
        2
    );
}

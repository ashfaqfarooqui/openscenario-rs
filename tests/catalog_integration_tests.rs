...
use openscenario_rs::catalog::{CatalogManager, CatalogLoader, ParameterSubstitutionEngine};
use openscenario_rs::types::catalogs::{
    files::CatalogFile,
    entities::ParameterDefinition,
    locations::{VehicleCatalogLocation, ControllerCatalogLocation},
    references::{VehicleCatalogReference, ParameterAssignment},
};
...
async fn test_catalog_file_parsing() {
    ...
    assert_eq!(catalog.catalog_name().as_literal().unwrap(), "TestVehicleCatalog");
...
async fn test_catalog_serialization_roundtrip() {
    ...
    assert_eq!(parsed_catalog.catalog_name().as_literal().unwrap(), "TestCatalog");
    assert_eq!(parsed_catalog.file_header.author.as_literal().unwrap(), "IntegrationTest");
}
...
async fn test_parameter_substitution_engine() {
    ...
    engine.set_parameter("VehicleName".to_string(), "SportsCar".to_string()).unwrap();
    engine.set_parameter("Color".to_string(), "Red".to_string()).unwrap();
    ...
    let result = engine.resolve_parameter_expression("Vehicle ${VehicleName} with max speed ${MaxSpeed} and color ${Color}").unwrap();
    assert_eq!(result, "Vehicle SportsCar with max speed 60.0 and color Red");
    
    // Test Value<T> resolution
    let speed_value: Value<f64> = Value::Parameter("MaxSpeed".to_string());
    let resolved_speed = engine.resolve_value(&speed_value).unwrap();
    assert_eq!(resolved_speed, 60.0);
    
    // Test expression resolution
    let expr_value: Value<String> = Value::Expression("${VehicleName} (${Color})".to_string());
...
async fn test_catalog_loader_with_temporary_files() {
    ...
    assert_eq!(catalog.catalog_name().as_literal().unwrap(), "TempTestCatalog");
    assert_eq!(catalog.vehicles().len(), 1);
...
async fn test_catalog_reference_creation() {
    ...
    let vehicle_ref = VehicleCatalogReference::new(
        "VehicleCatalog".to_string(),
        "SportsCar".to_string(),
    );
    
    assert_eq!(vehicle_ref.catalog_name.as_literal().unwrap(), "VehicleCatalog");
    ...
    assert_eq!(param_vehicle_ref.parameter_assignments.as_ref().unwrap().len(), 2);
    // Check first parameter
    let param_map = param_vehicle_ref.build_parameter_map(&HashMap::new()).unwrap();
...
fn test_catalog_error_handling() {
    ...
    let invalid_value: Value<f64> = Value::Parameter("NonExistentParam".to_string());
...
async fn test_catalog_location_directory_access() {
    ...
    assert_eq!(vehicle_location.directory.path.as_literal().unwrap(), "./test_vehicles");
    
    let controller_location = ControllerCatalogLocation::from_path("./test_controllers".to_string());
    assert_eq!(controller_location.directory.path.as_literal().unwrap(), "./test_controllers");
    ...
    assert_eq!(dir_ref1.path.as_literal().unwrap(), dir_ref2.path.as_literal().unwrap());
}
...
async fn test_real_catalog_file_structure() {
    ...
    if std::path::Path::new(vehicle_catalog_path).exists() {
        ...
        println!("Real catalog file not found at {}, skipping real file test", vehicle_catalog_path);
    }
}
...
async fn test_catalog_system_performance() {
    ...
    let _engines: Vec<ParameterSubstitutionEngine> = (0..100).map(|_| ParameterSubstitutionEngine::new()).collect();
    
    let duration = start.elapsed();
    
    // Should be able to create 10 managers and 100 engines reasonably quickly
    assert!(duration.as_millis() < 500, "Catalog system creation took too long: {:?}", duration);
}
...
fn test_catalog_reference_with_many_parameters() {
    ...
    assert_eq!(vehicle_ref.parameter_assignments.as_ref().unwrap().len(), 50);
    assert_eq!(vehicle_ref.catalog_name.as_literal().unwrap(), "LargeCatalog");
}

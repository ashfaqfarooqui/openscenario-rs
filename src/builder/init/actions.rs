...
use crate::types::{
    scenario::init::{Init, Actions, GlobalAction, Private, EnvironmentAction},
    positions::Position,
    environment::Environment,
    basic::Value,
};
use super::private::{PrivateActionBuilder, GlobalActionBuilder};
...
impl InitActionBuilder {
    ...
    pub fn add_teleport_action(mut self, entity_ref: &str, position: Position) -> Self {
        ...
        let private_index = self.private_actions
            .iter()
        ...
        if let Some(index) = private_index {
            ...
            let teleport_action = crate::types::scenario::init::PrivateAction {
                teleport_action: Some(crate::types::actions::movement::TeleportAction {
                    position,
                }),
                ..Default::default()
            };
            self.private_actions[index].private_actions.push(teleport_action);
        } else {
    ...
    pub fn add_speed_action(mut self, entity_ref: &str, speed: f64) -> Self {
        ...
        if let Some(index) = private_index {
            ...
            self.private_actions[index].private_actions.push(speed_action);
        } else {
            // Create new private action
            let speed_action = crate::types::scenario::init::PrivateAction {
                longitudinal_action: Some(crate::types::scenario::init::LongitudinalAction {
...
mod tests {
    ...
    fn test_init_action_builder_with_teleport() {
        ...
        assert!(init.actions.private_actions[0].private_actions[0].teleport_action.is_some());
    }
    ...
    fn test_init_action_builder_with_speed() {
        ...
        assert_eq!(init.actions.private_actions[0].private_actions.len(), 1);
        assert!(init.actions.private_actions[0].private_actions[0].longitudinal_action.is_some());
        
        let longitudinal = &init.actions.private_actions[0].private_actions[0].longitudinal_action.as_ref().unwrap();
    ...
    fn test_init_action_builder_multiple_actions_same_entity() {
        ...
        assert!(init.actions.private_actions[0].private_actions[1].longitudinal_action.is_some());
    }

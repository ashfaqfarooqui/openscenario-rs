...
use crate::types::{
    actions::movement::{
        LaneChangeAction, LaneChangeTarget, LaneChangeTargetChoice,
    ...
    basic::{Double, Boolean, Int, OSString},
...
impl ActionBuilder for LaneChangeActionBuilder {
    fn build_action(self) -> BuilderResult<PrivateAction> {
        ...
        Ok(PrivateAction {
            action: CorePrivateAction::LateralAction(
                LateralAction::lane_change(lane_change_action)
            ),
        })
    }
    ...
    fn validate(&self) -> BuilderResult<()> {
        if self.target.is_none() {
            return Err(BuilderError::validation_error("Lane change target is required"));
        }
        Ok(())
    }
}
...
impl ActionBuilder for LateralDistanceActionBuilder {
    ...
    fn validate(&self) -> BuilderResult<()> {
        if self.target_entity.is_none() {
            return Err(BuilderError::validation_error("Target entity is required for lateral distance action"));
        }
        Ok(())
    }
}
...
impl LaneOffsetActionBuilder {
    ...
    /// Set simple dynamics
    pub fn with_simple_dynamics(mut self, shape: DynamicsShape, max_lateral_acc: Option<f64>) -> Self {
...
impl ActionBuilder for LaneOffsetActionBuilder {
    fn build_action(self) -> BuilderResult<PrivateAction> {
        ...
        Ok(PrivateAction {
            action: CorePrivateAction::LateralAction(
                LateralAction::lane_offset(lane_offset_action)
            ),
        })
    }
    ...
    fn validate(&self) -> BuilderResult<()> {
        if self.target.is_none() {
            return Err(BuilderError::validation_error("Lane offset target is required"));
        }
        Ok(())
    }
}

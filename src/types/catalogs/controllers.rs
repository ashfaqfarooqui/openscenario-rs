...
impl ControllerProperties {
    ...
    where
        I: IntoIterator<Item = (String, Value<String>)>,
    {
    ...
    pub fn add_property(&mut self, name: String, value: Value<String>) {
    ...
    pub fn to_scenario_properties(&self) -> Properties {
        let scenario_properties = self.properties
            .iter()
            .map(|p| Property {
                name: p.name.clone(),
...
impl ControllerProperty {
    ...
    pub fn new(name: String, value: Value<String>) -> Self {
        Self { name, value }
    }
...
mod tests {
    ...
    fn test_controller_catalog_operations() {
        ...
        let controller1 = CatalogController::new("Controller1".to_string(), ControllerType::Movement);
        let controller2 = CatalogController::new("Controller2".to_string(), ControllerType::Lateral);
    ...
    fn test_controller_property_creation() {
        let property1 = ControllerProperty::with_literal(
            "testProp".to_string(),
            "testValue".to_string(),
        );
        let property2 = ControllerProperty::with_parameter(
            "paramProp".to_string(),
            "paramRef".to_string(),
        );
    ...
    fn test_catalog_controller_with_parameters() {
        ...
        assert_eq!(
            controller.parameter_declarations.as_ref().unwrap().parameter_declarations.len(),
            1
        );
    }
    ...
    fn test_to_scenario_controller() {
        let mut properties = ControllerProperties::default();
        properties.add_property("testProp".to_string(), Value::Literal("testValue".to_string()));
        ...
        assert_eq!(scenario_controller.name.as_literal().unwrap(), "TestController");
        assert_eq!(scenario_controller.controller_type, Some(ControllerType::Movement));

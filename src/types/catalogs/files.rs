...
use super::entities::{CatalogVehicle, CatalogController, CatalogPedestrian, CatalogMiscObject, CatalogEnvironment, CatalogManeuver, CatalogTrajectory, CatalogRoute};
...
impl CatalogFile {
    ...
    pub fn catalog_name(&self) -> &Value<String> {
        &self.catalog.name
    }
...
mod tests {
    ...
    fn test_catalog_file_creation() {
        ...
        assert_eq!(catalog.file_header.author.as_literal().unwrap(), "TestAuthor");
    ...
    fn test_catalog_file_default() {
        ...
        assert_eq!(catalog.catalog_name().as_literal().unwrap(), "DefaultCatalog");
        assert_eq!(catalog.file_header.author.as_literal().unwrap(), "openscenario-rs");
    }

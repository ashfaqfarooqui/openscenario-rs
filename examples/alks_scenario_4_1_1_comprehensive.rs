...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(params) = &scenario.parameter_declarations {
        println!("\n📋 ALKS Parameters ({} total):", params.parameter_declarations.len());
        println!("┌─────────────────────┬──────────────┬─────────────────────────┐");
    ...
    if let Some(entities) = &scenario.entities {
        println!("\n🚗 Vehicle Entities ({} total):", entities.scenario_objects.len());
        println!("┌─────────────────────┬─────────────────────────────────────────┐");
    ...
    if let Some(storyboard) = &scenario.storyboard {
        ...
        for story in &storyboard.stories {
            if let Some(name) = story.name.as_literal() {
                ...
                println!("│   Acts: {:2} (Three-phase ALKS testing sequence)          │", story.acts.len());
                for (i, act) in story.acts.iter().enumerate() {
                    if let Some(act_name) = act.name.as_literal() {
                        ...
                        println!("│       Maneuver Groups: {:2}                              │", act.maneuver_groups.len());
                    }
                }
            }
        }
...
fn main() {
    ...
    println!("💡 Run with: cargo run --example alks_scenario_4_1_1_comprehensive --features builder");
